...
jest.mock('../../repository/organization/authenticated_user_repository', () => ({
...
describe('Execution', () => {
  ...
  describe('setup', () => {
    it('sets tokenUser and throws if getUserFromToken returns null', async () => {
      ...
      const e = buildExecution(undefined, {});
      await expect(e.setup(branchRepository, setupIssuePort, setupOrganizationPort, setupConfigurationPort)).rejects.toThrow('Failed to get user from token');
    });
    ...
    it('sets issueNumber from issue when isIssue and not single action', async () => {
      ...
      await e.setup(branchRepository, setupIssuePort, setupOrganizationPort, setupConfigurationPort);
    ...
    it('setup throws when getLabels fails and not INITIAL_SETUP', async () => {
      ...
      await expect(e.setup(branchRepository, setupIssuePort, setupOrganizationPort, setupConfigurationPort)).rejects.toThrow('Fatal label error');
    });
  });
});
