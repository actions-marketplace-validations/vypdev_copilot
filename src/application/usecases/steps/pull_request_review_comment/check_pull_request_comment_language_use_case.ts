    ...
    async invoke(param: Execution): Promise<Result[]> {
        ...
        const status =
            ...
            typeof (checkResponse as Record<string, unknown>).status === 'string'
                ...
                : '';
        logDebugInfo(`CheckPullRequestCommentLanguage: language check status=${status}.`);
        if (status === 'done') {
            results.push(
                new Result({
                    id: this.taskId,
        ...
        logDebugInfo(`CheckPullRequestCommentLanguage: translation received. translatedText length=${translatedText.length}. Full translated text:\n${translatedText}`);
