...
export async function prepareReleaseBranch(
    ...
    const linkResult = await repository.createLinkedBranch(
        param.owner,
        param.repo,
    ...
    if (commitPrefix) reminders.push(`Commit the needed changes with this prefix:\n> ${fence}\n>${commitPrefix}\n> ${fence}`);
...
async function buildCommitPrefix(param: Execution, branchName: string): Promise<string> {
