...
describe('IssueUseCase', () => {
  ...
  it('when permissions fail, pushes permission result and close not allowed then returns', async () => {
    ...
    const useCase = new IssueUseCase({ setTaskPriority: jest.fn().mockResolvedValue(true) }, { getAllMembers: jest.fn().mockResolvedValue([]), getRandomMembers: jest.fn() }, { getId: jest.fn().mockResolvedValue('id') }, { moveIssueToColumn: jest.fn(), setTaskPriority: jest.fn(), setTaskSize: jest.fn() }, { linkContentId: jest.fn() }, { getTitle: jest.fn(), updateTitleIssueFormat: jest.fn(), updateTitlePullRequestFormat: jest.fn() }, { getCurrentAssignees: jest.fn(), assignMembersToIssue: jest.fn() }, { closeIssue: jest.fn(), addComment: jest.fn() }, { setIssueType: jest.fn() }, { getDescription: jest.fn() }, { addComment: jest.fn(), openIssue: jest.fn() }, { getListOfBranches: jest.fn(), removeBranch: jest.fn() }, { formatBranchName: jest.fn() }, { fetchRemoteBranches: jest.fn(), getListOfBranches: jest.fn(), getCommitTag: jest.fn(), createLinkedBranch: jest.fn(), manageBranches: jest.fn(), removeBranch: jest.fn(), formatBranchName: jest.fn() }, { executeWorkflow: jest.fn() }, { taskId: 'RecommendStepsUseCase', invoke: mockRecommendStepsInvoke }, { taskId: 'AnswerIssueHelpUseCase', invoke: mockAnswerIssueHelpInvoke })
