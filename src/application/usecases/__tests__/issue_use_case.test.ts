...
describe('IssueUseCase', () => {
  ...
  it('when permissions fail, pushes permission result and close not allowed then returns', async () => {
    ...
    const useCase = new IssueUseCase({ setTaskPriority: jest.fn().mockResolvedValue(true) }, { getAllMembers: jest.fn().mockResolvedValue([]), getRandomMembers: jest.fn() }, { getId: jest.fn().mockResolvedValue('id') }, { linkContentId: jest.fn(), moveIssueToColumn: jest.fn(), setTaskPriority: jest.fn(), setTaskSize: jest.fn() }, { getTitle: jest.fn(), updateTitleIssueFormat: jest.fn(), updateTitlePullRequestFormat: jest.fn() }, { getCurrentAssignees: jest.fn(), assignMembersToIssue: jest.fn() })
