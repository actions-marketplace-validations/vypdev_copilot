...
jest.mock('../issue_use_case_composition_root', () => ({
  createIssueUseCaseCompositionRoot: jest.fn(() => ({ invoke: mockIssueInvoke })),
}));

jest.mock('../pull_request_use_case_composition_root', () => ({
  createPullRequestUseCaseCompositionRoot: jest.fn(() => ({ invoke: mockPullRequestInvoke })),
}));
...
jest.mock('../../../application/usecases/steps/commit/bugbot/bugbot_autofix_use_case');
jest.mock('../../../application/usecases/steps/commit/bugbot/detect_bugbot_fix_intent_use_case');
...
jest.mock('../../../application/usecases/steps/issue_comment/check_issue_comment_language_use_case');
...
describe('main run route composition root', () => {
  ...
  it('shares route-scoped Bugbot, agent, and Git commit capabilities', () => {
    ...
    expect(CheckIssueCommentLanguageUseCase).toHaveBeenCalledWith(mockIssue, mockFindings);
    expect(DetectBugbotFixIntentUseCase).toHaveBeenCalledWith(
      mockPullRequest,
      mockFindings,
      mockContext,
    );
    expect(ThinkUseCase).toHaveBeenCalledWith(expect.anything(), expect.anything(), mockFindings);

    const gitCommit = (GitCommitAdapter as jest.MockedClass<typeof GitCommitAdapter>).mock.instances[0];
    expect(GitCommitAdapter).toHaveBeenCalledTimes(1);
    expect(BugbotAutofixUseCase).toHaveBeenCalledWith(mockFixer, mockContext, gitCommit);
    ...
    expect(IssueCommentUseCase).toHaveBeenCalledWith(
      ...
      expect.anything(),
      expect.anything(),
      { issueComments: mockIssue, pullRequestComments: mockPullRequest },
      gitCommit,
    );
  });

  it('exposes one handler for every executable route', () => {
    const handlers = createMainRunRouteCompositionRoot({} as ProjectBoardCommandPort);
    ...
    Object.values(handlers).forEach((handler) => expect(handler).toEqual(expect.any(Function)));
  });
  ...
  it('binds every route to its matching use case', async () => {
    ...
    const singleAction = (SingleActionUseCase as jest.MockedClass<typeof SingleActionUseCase>).mock.instances[0];
    expect(singleAction.invoke).toHaveBeenCalledWith(execution);

    await handlers['issue-comment'](execution);
    const issueCommentInstances = (IssueCommentUseCase as jest.MockedClass<typeof IssueCommentUseCase>).mock.instances;
    expect(issueCommentInstances[issueCommentInstances.length - 1].invoke).toHaveBeenCalledWith(execution);
    ...
    const reviewComment = (PullRequestReviewCommentUseCase as jest.MockedClass<typeof PullRequestReviewCommentUseCase>).mock.instances[0];
    ...
    const commit = (CommitUseCase as jest.MockedClass<typeof CommitUseCase>).mock.instances[0];
