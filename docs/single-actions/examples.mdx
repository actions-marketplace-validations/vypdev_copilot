...
Common options: **`-t`** / **`--token`** (PAT), **`-d`** / **`--debug`**, **`--opencode-server-url`**, **`--opencode-model`**. See [Workflow & CLI](/single-actions/workflow-and-cli) and [Testing OpenCode Plan Locally](/testing-opencode-plan-locally).
