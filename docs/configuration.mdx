...
<Tabs sync={false}>
  <Tab title="Authentication & AI" icon="key">
    ...
    - `bugbot-fix-verify-commands`: Comma-separated commands to run after Bugbot autofix (e.g. `pnpm run build, pnpm test, pnpm run lint`). When a user asks to fix findings from an issue or PR comment, OpenCode applies fixes and these commands run before the action commits and pushes; if any fails, no commit is made. Default: empty (only OpenCode's run is used). See [Features → Bugbot autofix](/features#ai-features-opencode).
