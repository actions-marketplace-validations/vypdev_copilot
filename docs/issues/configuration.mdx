...
- `agent-transport`: `server`, `sdk`, or `cli` (default: `server`)
- `agent-model`: provider-specific model or executable identifier (defaults to `opencode-model`)
- `agent-command`: literal executable plus arguments for CLI transport (required for `cli`)

Current execution matrix:

| Provider | `server` | `sdk` | `cli` |
| --- | --- | --- | --- |
| OpenCode | supported; managed startup preserves the legacy lifecycle | rejected until an SDK adapter is verified | supported when explicitly configured |
| Cursor | rejected until a server adapter is verified | rejected until an SDK adapter is verified | supported through the configured CLI |
| Codex | rejected until a server adapter is verified | rejected until an SDK adapter is verified | supported through the configured CLI |

CLI commands are executed without a shell. Shell operators, substitutions, pipes, redirects, and non-literal command tokens are rejected. The prompt is sent through standard input, and stdout is treated as the agent response. Configure credentials through the runner/provider environment; never include tokens in `agent-command`.
