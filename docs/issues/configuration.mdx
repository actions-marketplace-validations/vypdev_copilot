...
- `agent-provider`: `opencode`, `cursor`, or `codex` (default: `opencode`)
- `agent-transport`: `server` or `cli` (default: `server`)
- `agent-model`: provider-specific model or executable identifier (defaults to `opencode-model`)
- `agent-command`: optional literal executable plus arguments for CLI transport;
  defaults are `opencode`, `cursor-agent`, or `codex` according to the provider

Current execution matrix:

| Provider | `server` | `cli` |
| --- | --- | --- |
| OpenCode | supported; managed startup preserves the legacy lifecycle | experimental generic command transport; no provider-specific flags are added |
| Cursor | rejected | experimental generic command transport; provider-specific integration is not wired or end-to-end verified |
| Codex | rejected | experimental generic command transport; provider-specific integration is not wired or end-to-end verified |

SDK transport is not part of the current public contract. It remains a future
design option in the explicitly non-active multi-agent proposal.

CLI commands are executed without a shell. Shell operators, substitutions,
pipes, redirects, and non-literal command tokens are rejected. The generic
transport sends the prompt through standard input and treats stdout as the agent
response. A default executable name does not prove that a provider's current
CLI accepts this protocol. Use CLI transport only after verifying the selected
non-interactive command in the runner environment, preferably by setting
`agent-command` explicitly. Configure credentials through the runner/provider
environment; never include tokens in `agent-command`.

Legacy OpenCode inputs remain supported:

#### AI (OpenCode)
- `opencode-server-url`: OpenCode server URL (default: "http://localhost:4096"). See [OpenCode (AI)](/opencode-integration).
