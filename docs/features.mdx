...
<CardGroup cols={3}>
  ...
  <Card title="Pull request events" icon="git-pull-request" href="/pull-requests">
  ...
  <Card title="Push events" icon="git-commit">
    On every push: commit notifications on the issue, prefix check, reopen option, size & progress labels (with OpenCode). Details below.
  </Card>
</CardGroup>
...
Configuration: `opencode-server-url`, `opencode-model`, and optionally `opencode-start-server` (action starts and stops OpenCode in the job). For bugbot autofix, use `bugbot-fix-verify-commands` to list commands to run after fixes (e.g. `pnpm run build, pnpm test, pnpm run lint`). See [Bugbot](/bugbot) and [OpenCode (AI)](/opencode-integration).
