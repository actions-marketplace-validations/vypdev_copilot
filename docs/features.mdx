...
Configuration: `opencode-server-url`, `opencode-model`, and optionally `opencode-start-server` (action starts and stops OpenCode in the job). For bugbot autofix, use `bugbot-fix-verify-commands` to list commands to run after fixes (e.g. `npm run build, npm test, npm run lint`). See [Bugbot](/bugbot) and [OpenCode (AI)](/opencode-integration).
